    ...
    [err, assessmentQuestions] = await to(assessment_questions.findAll(
      {
        ...
        include:
          [
            {
              model: questions,
              attributes:['id','question_type', 'statement', 'mime_type','hint','difficulty_level','complexity_level','knowledge_level','proficiency_level','blooms_taxonomy','skill_id','estimated_time','correct_answer_score','level_id','tags','subject_id','s3url', ['s3url', 's3_asset_urls'], 'grade_id',  'topic_id', 'sub_strand_id', 'strand_id', 'lo_ids'],
              include: [
